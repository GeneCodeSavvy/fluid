"""
TestCase: Access hostPath data via Fluid accelerate
DDC Engine: Alluxio
Steps:
1. create Dataset(local://) & AlluxioRuntime with nodeAffinity
...
def main():
    ...
    dataset = fluidapi.assemble_dataset(
        "hostpath",
        node_affinity_key=node_label_key,
        node_affinity_value=node_label_value
    ).set_namespaced_name(namespace, name)

    runtime = fluidapi.assemble_runtime("hostpath").set_namespaced_name(namespace, name)

    flow = TestFlow("HostPath - Basic data access via local:// mount")
